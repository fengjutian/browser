...
func Open(ctx context.Context, path string) (*sql.DB, error) {
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil { return nil, fmt.Errorf("create data directory: %w", err) }
	db, err := sql.Open("sqlite", path+"?_pragma=foreign_keys(1)&_pragma=journal_mode(WAL)&_pragma=busy_timeout(5000)")
	if err != nil { return nil, err }
	if err = db.PingContext(ctx); err != nil { _ = db.Close(); return nil, err }
	if err = migrate(ctx, db); err != nil { _ = db.Close(); return nil, err }
	return db, nil
}
...
CREATE INDEX IF NOT EXISTS idx_documents_created_at ON documents(created_at DESC);`
	_, err := db.ExecContext(ctx, schema); return err
}
