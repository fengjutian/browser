...
mod windows_guard {
    ...
    use webview2_com::Microsoft::Web::WebView2::Win32::{
        ...
        COREWEBVIEW2_WEB_ERROR_STATUS_CERTIFICATE_NO_REVOCATION_MECHANISM,
        COREWEBVIEW2_WEB_ERROR_STATUS_CERTIFICATE_REVOKED,
        COREWEBVIEW2_WEB_ERROR_STATUS_CERTIFICATE_UNABLE_TO_CHECK_REVOCATION,
    ...
    fn pending() -> &'static Mutex<HashMap<String, Pending>> { PENDING.get_or_init(Default::default) }
    ...
    fn status_message(status: COREWEBVIEW2_WEB_ERROR_STATUS) -> &'static str {
        ...
        else if status == COREWEBVIEW2_WEB_ERROR_STATUS_CERTIFICATE_NO_REVOCATION_MECHANISM { "证书没有可用的吊销检查机制" }
        else if status == COREWEBVIEW2_WEB_ERROR_STATUS_CERTIFICATE_UNABLE_TO_CHECK_REVOCATION { "无法检查证书吊销状态" }
        else { "TLS 证书验证失败" }
    }
    ...
    pub fn attach<R: Runtime>(app: &AppHandle<R>, label: &str) {
        ...
        let _ = view.with_webview(move |platform| {
            ...
            let handler = ServerCertificateErrorDetectedEventHandler::create(Box::new(move |_sender, args| {
                ...
                let mut uri = CoTaskMemPWSTR::default();
                unsafe { args.RequestUri(uri.as_mut().as_pwstr() as *const _ as *mut _) }?;
                ...
                match pending().lock() {
                    Ok(mut map) => { map.insert(request_id.clone(), Pending { args, deferral, label: event_label.clone() }); }
                    Err(_) => {
                        unsafe { args.SetAction(COREWEBVIEW2_SERVER_CERTIFICATE_ERROR_ACTION_CANCEL)?; deferral.Complete()?; }
                        return Ok(());
                    }
                }
    ...
    pub fn respond<R: Runtime>(app: AppHandle<R>, request_id: String, allow: bool) -> Result<(), String> {
        let item = pending().lock().map_err(|_| "certificate request registry poisoned")?
        ...
        if allow {
            ...
            std::thread::spawn(move || {
                ...
                if let Some(view) = delayed_app.get_webview(&item.label) {
