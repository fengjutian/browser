...
import { closeNativeTab, hasNativeTab, hideNativeTab, navigateHistory, openNativeTab, reloadNativeTab, resizeNativeTab, showNativeTab } from '../../services/nativeBrowser'
...
export function BrowserPage() {
  ...
  async function save() {
    ...
    try {
      ...
      const created = await saveDocument({ title: active.title, url: active.url || 'about:blank', markdown: 'Captured by Reader pipeline.', tags: ['Inbox'] })
  ...
  return <div className="browser-page">{contextHolder}
    ...
    <div className="browser-toolbar"><Space><Button type="text" icon={<ArrowLeftOutlined/>} onClick={() => void navigateHistory(active.id,-1)}/><Button type="text" icon={<ArrowRightOutlined/>} onClick={() => void navigateHistory(active.id,1)}/><Button type="text" loading={active.loading} icon={<ReloadOutlined/>} onClick={() => void reloadNativeTab(active.id)}/></Space><form onSubmit={event => void navigate(event)}><Input prefix={<SafetyCertificateOutlined/>} suffix={<StarOutlined/>} value={address} onChange={event=>setAddress(event.target.value)} placeholder="搜索或输入网址"/></form><Tag icon={<SafetyCertificateOutlined/>} color="green">43</Tag><Button type={aiOpen?'primary':'text'} ghost={aiOpen} icon={<RobotOutlined/>} onClick={()=>setAiOpen(value=>!value)}/></div>
    <div className="browser-content"><div className="web-surface" ref={surfaceRef}>{!nativeMode && (active.url ? <ReaderPreview/> : <NewTab address={address} setAddress={setAddress} navigate={navigate}/>)}</div>{aiOpen&&<AssistantPanel close={()=>setAiOpen(false)} save={save}/>}</div>
  </div>
}
