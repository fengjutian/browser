import { Empty, Input, List, Segmented, Select, Space, Statistic, Tag, Typography } from 'antd'
...
export function SearchPage() {
  ...
  return <section className="page"><PageHeader eyebrow="LOCAL SEARCH" title="找回你读过的内容" description="搜索保存在本地 SQLite 中的真实网页内容。"/><Input size="large" prefix={<SearchOutlined/>} value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索标题、正文或标签" suffix={`${filtered.length} 条结果${durationMs !== null ? ` · ${durationMs}ms` : ''}`}/>
    ...
    <List loading={loading} className="search-results" dataSource={filtered} locale={{ emptyText: <Empty description={emptyText}/> }} renderItem={item => <List.Item onClick={() => setSelected(item)} className="search-result-clickable" actions={[<RightOutlined key="open"/>]}><List.Item.Meta avatar={<span className="result-icon"><FileSearchOutlined/></span>} title={<Highlight text={item.title} terms={terms}/>} description={<><Space size={4}>{item.tags.map(tag => <Tag color={tagFilter.includes(tag) ? 'gold' : 'green'} key={tag}>{tag}</Tag>)}</Space><Typography.Paragraph ellipsis={{ rows: 2, expandable: false }}><Highlight text={extractSummary(item.summary, item.markdown)} terms={terms}/></Typography.Paragraph></>}/></List.Item>}/>
