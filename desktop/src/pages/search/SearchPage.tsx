...
export function SearchPage() {
  ...
  return <section className="page"><PageHeader eyebrow="LOCAL SEARCH" title="找回你读过的内容" description="搜索保存在本地 SQLite 中的真实网页内容。"/><Input size="large" prefix={<SearchOutlined/>} value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索标题、正文或标签" suffix={`${results.length} 条结果`}/><List loading={loading} className="search-results" dataSource={results} locale={{ emptyText: <Empty description={emptyText}/> }} renderItem={item => <List.Item onClick={() => setSelected(item)} className="search-result-clickable" actions={[<RightOutlined key="open"/>]}><List.Item.Meta avatar={<span className="result-icon"><FileSearchOutlined/></span>} title={item.title} description={<><Space size={4}>{item.tags.map(tag => <Tag key={tag}>{tag}</Tag>)}</Space><Typography.Paragraph ellipsis={{ rows: 2, expandable: false }}>{extractSummary(item.summary, item.markdown)}</Typography.Paragraph></>}/></List.Item>}/><DocumentDetailDrawer document={selected} onClose={() => setSelected(null)} onDeleted={id => setResults(items => items.filter(item => item.id !== id))}/></section>
}
